<div class="space-y-8" x-data="{ showAddDialog: @entangle('showAddDialog'), showAdjustDialog: @entangle('showAdjustDialog') }">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
        <div>
            <h1 class="text-3xl font-black text-foreground tracking-tight">Inventory</h1>
            <p class="text-muted-foreground mt-1 text-lg font-medium">Track ingredients, stock levels, and low-stock alerts</p>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-6 px-2">
        <div class="p-8 bg-card border border-border rounded-[3rem] shadow-glow-sm relative overflow-hidden group">
            ...
            <p class="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] mb-3 relative z-10">Total Ingredients</p>
            <div class="text-foreground font-black text-3xl tracking-tighter relative z-10">{{ $stats['total'] }} <span class="text-xs text-muted-foreground uppercase tracking-widest ml-1">Items</span></div>
        </div>
        ...
        <div class="p-8 bg-card border border-border rounded-[3rem] shadow-glow-sm relative overflow-hidden group">
            ...
            <p class="text-[10px] font-black {{ $stats['low'] > 0 ? 'text-rose-700 dark:text-rose-400' : 'text-emerald-700 dark:text-emerald-400' }} uppercase tracking-[0.3em] mb-3 relative z-10">Low Stock</p>
            <div class="{{ $stats['low'] > 0 ? 'text-rose-700 dark:text-rose-400' : 'text-emerald-700 dark:text-emerald-400' }} font-black text-3xl tracking-tighter relative z-10">
                {{ $stats['low'] }} <span class="text-xs uppercase tracking-widest ml-1">{{ $stats['low'] > 0 ? 'Need restock' : 'All good' }}</span>
            </div>
        </div>
        ...
        <div class="p-8 bg-card border border-border rounded-[3rem] shadow-glow-sm relative overflow-hidden group">
            ...
            <p class="text-[10px] font-black text-blue-700 dark:text-blue-400 uppercase tracking-[0.3em] mb-3 relative z-10">Unit Types</p>
            <div class="text-blue-700 dark:text-blue-400 font-black text-3xl tracking-tighter relative z-10">{{ $stats['units'] }} <span class="text-xs uppercase tracking-widest ml-1">In use</span></div>
        </div>
        ...
        <div class="p-8 bg-card border border-border rounded-[3rem] shadow-glow-sm relative overflow-hidden group">
            ...
            <p class="text-[10px] font-black text-purple-700 dark:text-purple-400 uppercase tracking-[0.3em] mb-3 relative z-10">Healthy Stock</p>
            <div class="text-purple-700 dark:text-purple-400 font-black text-3xl tracking-tighter relative z-10">{{ $stats['healthy'] }} <span class="text-xs uppercase tracking-widest ml-1">Items</span></div>
        </div>
    </div>
        ...
        <div class="flex flex-col md:flex-row gap-3">
            <div class="relative flex-1">
                ...
                <input
                    ...
                    class="pl-14 h-14 w-full bg-background/50 border border-border/50 rounded-[1.75rem] text-sm font-medium focus:bg-background focus:ring-4 focus:ring-primary/10 transition-all outline-none placeholder:text-muted-foreground/60" aria-label="Search ingredients"
                />
            </div>
            <div class="flex items-center gap-3 py-2 px-6 bg-background/50 border border-border/50 rounded-[1.75rem] shrink-0">
                <label class="flex items-center gap-3 cursor-pointer group">
                    ...
                    <span class="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] group-hover:text-foreground transition-colors">Low Stock Only</span>
                </label>
            </div>
        </div>
    </div>
    ...
    <div class="mx-2 bg-card border border-border rounded-[3.5rem] overflow-hidden shadow-glow-sm border-t-0 p-1">
        <div class="p-10 border-b border-border bg-muted/[0.15] flex items-center justify-between rounded-t-[3.25rem]">
            <div>
                <h3 class="text-xl font-black text-foreground tracking-tight">Ingredients</h3>
                <p class="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] mt-1">Stock levels and alert thresholds</p>
            </div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-muted/10">
                        <th class="text-left py-6 px-10 text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">Ingredient</th>
                        <th class="text-left py-6 px-10 text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">Stock Level</th>
                        <th class="text-left py-6 px-10 text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">Unit</th>
                        <th class="text-left py-6 px-10 text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">Alert Threshold</th>
                        <th class="text-right py-6 px-10 text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">Actions</th>
                    </tr>
                </thead>
                            ...
                            <td class="py-6 px-10">
                                <div class="flex items-center gap-5">
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <p class="text-[9px] font-bold text-muted-foreground uppercase tracking-widest opacity-70">ID: {{ str_pad($ing->id, 6, '0', STR_PAD_LEFT) }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="py-6 px-10">
                                <div class="space-y-2 max-w-[120px]">
                                    <div class="flex items-center justify-between text-[10px] font-black uppercase tracking-tight">
                                        <span class="{{ $isLow ? 'text-rose-700 dark:text-rose-400 pulse' : 'text-foreground' }}">{{ (float)$ing->stock }} {{ $ing->unit }}</span>
                                        <span class="text-muted-foreground">{{ round($percent) }}%</span>
                                    </div>
                            ...
                            <td class="py-6 px-10 text-right">
                                <div class="flex justify-end gap-3">
                                    ...
                                    <button
                                        ...
                                        wire:confirm="Delete {{ $ing->name }} permanently?"
                                        aria-label="Delete {{ $ing->name }}"
                                        class="size-11 rounded-[1.25rem] flex items-center justify-center text-rose-600/70 dark:text-rose-400/70 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-500/10 transition-colors border border-transparent hover:border-rose-500/20"
                                    >
                        ...
                        <tr>
                            <td colspan="5" class="py-32 text-center relative overflow-hidden">
                                ...
                                <div class="relative z-10">
                                    <h3 class="text-2xl font-black text-foreground uppercase tracking-tight">No ingredients yet</h3>
                                    <p class="text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em] mt-3">Add your first ingredient to start tracking stock</p>
    ...
    >
        ...
        >
            <div class="p-12 text-center border-b border-dashed border-border/50">
                <button @click="showAdjustDialog = false" aria-label="Close" class="absolute right-10 top-10 size-12 flex items-center justify-center rounded-full bg-muted/30 hover:bg-muted transition-all">
                ...
                <h2 class="text-3xl font-black text-foreground uppercase tracking-tighter">Adjust Stock</h2>
                <p class="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] mt-4">
            ...
            <div class="p-12 space-y-8">
                <div class="space-y-3">
                    <label class="text-[10px] font-black text-foreground uppercase tracking-[0.3em] pl-1">Adjustment Type</label>
                    <div class="grid grid-cols-3 gap-3">
                        @foreach(['restock' => 'Restock', 'wastage' => 'Wastage', 'correction' => 'Correction'] as $key => $label)
                            <button 
                                type="button" 
                ...
                <div class="space-y-3">
                    <label class="text-[10px] font-black text-foreground uppercase tracking-[0.3em] pl-1" for="adjust-amount">Quantity ({{ $selectedIngredient?->unit }})</label>
                    <div class="relative">
                        <i data-lucide="calculator" class="absolute left-6 top-1/2 -translate-y-1/2 size-6 opacity-20"></i>
                        <input type="number" id="adjust-amount" wire:model="adjustAmount" class="pl-16 h-20 w-full rounded-[2rem] bg-muted/50 border border-border/50 text-2xl font-black outline-none focus:bg-background focus:ring-4 focus:ring-blue-500/10 transition-all">
                    </div>
                </div>

                <div class="space-y-3">
                    <label class="text-[10px] font-black text-foreground uppercase tracking-[0.3em] pl-1" for="adjust-reason">Reason</label>
                    <input type="text" id="adjust-reason" wire:model="adjustReason" placeholder="Stock delivery, damage info, etc." class="h-14 w-full px-7 rounded-[1.5rem] bg-muted/50 border border-border/50 text-xs font-black uppercase transition-all outline-none focus:bg-background">
                </div>
    ...
    >
        ...
        >
            <div class="p-12 text-center border-b border-dashed border-border/50 relative">
                <button @click="showAddDialog = false" aria-label="Close" class="absolute right-10 top-10 size-12 flex items-center justify-center rounded-full bg-muted/30 hover:bg-muted transition-all">
                    <i data-lucide="x" class="size-5 text-slate-500"></i>
                </button>
                ...
                <h2 class="text-3xl font-black text-foreground uppercase tracking-tighter">{{ $editingIngredientId ? 'Edit Ingredient' : 'Add Ingredient' }}</h2>
                <p class="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] mt-4">Ingredient details</p>
            </div>

            <div class="p-12 space-y-8">
                <div class="space-y-3">
                    <label class="text-[10px] font-black text-foreground uppercase tracking-[0.3em] pl-1" for="ing-name">Name</label>
                    <div class="relative">
                        <i data-lucide="edit-3" class="absolute left-6 top-1/2 -translate-y-1/2 size-6 opacity-20"></i>
                        <input type="text" id="ing-name" wire:model="name" class="pl-16 h-16 w-full rounded-[1.75rem] bg-muted/50 border border-border/50 text-sm font-medium transition-all outline-none focus:ring-4 focus:ring-blue-500/10">
                    </div>
                </div>

                <div class="grid grid-cols-2 gap-6">
                    <div class="space-y-3">
                        <label class="text-[10px] font-black text-foreground uppercase tracking-[0.3em] pl-1" for="ing-stock">Current Stock</label>
                        <input type="number" step="0.001" id="ing-stock" wire:model="stock" class="h-16 w-full px-6 rounded-[1.75rem] bg-muted/50 border border-border/50 text-sm font-black transition-all outline-none focus:ring-4 focus:ring-blue-500/10">
                    </div>
                    <div class="space-y-3">
                        <label class="text-[10px] font-black text-foreground uppercase tracking-[0.3em] pl-1" for="ing-unit">Unit</label>
                        <select wire:model="unit" id="ing-unit" class="h-16 w-full px-6 rounded-[1.75rem] bg-muted/50 border border-border/50 text-[10px] font-black uppercase tracking-[0.2em] transition-all outline-none focus:ring-4 focus:ring-blue-500/10 appearance-none">
                ...
                <div class="space-y-3 p-8 bg-rose-500/[0.03] rounded-[2.5rem] border border-dashed border-rose-500/20">
                    <label class="text-[10px] font-black text-rose-700 dark:text-rose-400 uppercase tracking-[0.3em] pl-1" for="ing-min-level">Low Stock Alert Level</label>
                    <div class="relative">
                        <i data-lucide="alert-triangle" class="absolute left-6 top-1/2 -translate-y-1/2 size-6 text-rose-500 opacity-40"></i>
                        <input type="number" step="0.001" id="ing-min-level" wire:model="min_level" class="pl-16 h-16 w-full rounded-[1.5rem] bg-background border border-rose-500/30 text-rose-700 dark:text-rose-400 text-sm font-black outline-none focus:ring-4 focus:ring-rose-500/10 transition-all">
                    </div>
                    <p class="text-[9px] font-bold text-rose-700/80 dark:text-rose-400/80 uppercase tracking-[0.2em] mt-3">You will be alerted when stock falls below this level</p>
                </div>
